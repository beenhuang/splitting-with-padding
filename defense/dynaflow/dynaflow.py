...
def defend(trace):
    ...
    switch_sizes=[400, 1200, 2000, 2800, 3600, 4400, 5200]
    m=1.2
    memory=100
    max_packet_size=512

    trace_new = [[x[0], x[1], max_packet_size] for x in trace]

    return dynaflow(trace_new, subseq_length, \
                               first_time_gap, \
                               poss_time_gaps, \
                               switch_sizes, \
                               m, \
...
def dynaflow(packets, subseq_length, \
                      first_time_gap, \
                      poss_time_gaps, \
                      switch_sizes, \
                      m, \
...
def create_end_sizes(m):
    MAX_IDX=9999
    MAX_ENDSIZE=10000000
