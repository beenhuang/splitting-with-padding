...
class SWP():
    ...
    def __split_trace(self, trace, pkt_path):
        ...
        for idx, pkt in enumerate(pkt_path):
            if pkt == 0:
                trace_0.append(trace[idx])
            elif pkt == 1:
                trace_1.append(trace[idx])
            elif pkt == 2:
                trace_2.append(trace[idx])    

        #print(f"t0:{len(trace_0)}, t1:{len(trace_1)}, t2:{len(trace_2)}")        

        if paths == 1: # 2 paths
            return [trace_0, trace_1]    
        else: # 3 paths
            return [trace_0, trace_1, trace_2]    
